...
To update later: open the file on GitHub, click the pencil icon, edit, commit. Pages redeploys on its own within a minute or two.
...
|---|---|---|
| `SCORE_DECAY_KM` | 1200 | Base score = 100 × e^(−km / this). Raise to be more forgiving. |
| `PERFECT_RADIUS_KM` | 20 | Guesses this close get a full 100. |
| `MULTIPLIER_FIRST` / `LAST` | 1 / 2 | Round 1 is ×1.0, round 6 is ×2.0, linear between. Max total 900. |
| `HINT_FACTOR` | 0.5 | A hint multiplies that round's multiplier by this. |
| `WARM_THRESHOLD_KM` | 2000 | Warm/cold hint says "warm" inside this distance. |
| `MAX_KM_PER_PX` | 525/390 | Zoom cap (see below). |
| `LAUNCH_DATE` | 2026-09-01 | Puzzle #1 and the start of the archive. |

What the scoring curve looks like at the default 1200:
...
**Daily picks: country first, then city.** Every sovereign country has an equal chance of being picked (dependent territories get half weight). Then a city inside that country is picked, with bigger cities only mildly favored (weight = population^0.3). Knowing the country is the main skill, and knowing the exact city earns the last few points. Big famous cities show up only occasionally. Each day has at most two places per continent and no repeated countries. The same country will not come back within 14 days, and the same city will not come back within 60.
...
**Share grid** (uses the proximity score before multipliers, so hints and later rounds do not change the colors):
🎯 95+ · 🟩 75+ · 🟨 50+ · 🟧 25+ · 🟥 5+ · ⬛ under 5. A 💡 count is added when hints were used.
...
Map engine: MapLibre GL JS (BSD-3). Imagery: Esri, Maxar, Earthstar Geographics, and the GIS User Community. Places: GeoNames (CC BY 4.0). Country metadata: mledoze/countries (ODbL). Fonts: Syne and Space Mono (Google Fonts).
